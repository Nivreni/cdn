(function (win, doc) {
    ...
    (function () {
        var DAY_BG = 'https://cdn.jsdmirror.com/gh/Nivreni/cdn@refs/heads/main/images/bg22.jpg';
        var NIGHT_BG = 'https://cdn.jsdmirror.com/gh/Nivreni/cdn@refs/heads/main/images/bg22-night.jpg';
        var DAY_START = 6, DAY_END = 18;
        var applied = '';

        function pick() {
            var m = win.location.search.match(/[?&]bg=(day|night)\b/);
            if (m) { return m[1]; }
        ...
        function applyNow(mode) {
            ...
            if (mode === 'night') {
                ...
                probe.onload = function () {
                    if (applied !== 'night') { return; }
                    if (de.classList) { de.classList.add('yc-night'); }
                    if (doc.body) { doc.body.style.backgroundImage = 'url("' + NIGHT_BG + '")'; }
                };
                probe.src = NIGHT_BG;
            } else {
                if (de.classList) { de.classList.remove('yc-night'); }
                if (doc.body) { doc.body.style.backgroundImage = ''; }
            }
        }
        ...
        function boot() {
            var mode = pick();
            applyNow(mode);
            addEvent(win, 'load', function () { applyNow(mode); });
        }

        if (doc.body) { boot(); }
        else { addEvent(doc, 'DOMContentLoaded', boot); }
    }());
    ...
    (function () {
        ...
        addEvent(win, 'scroll', resetX, true);
        addEvent(doc, 'DOMContentLoaded', resetX);
        addEvent(win, 'load', resetX);
    }());
