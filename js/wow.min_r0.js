...
(function (win, doc) {
    ...
    /* ==========================================================
     * 登录提示 Toast —— 参考力 zashboard 的右上角通知样式
     * 把原生 #msgpid 的错误文本转成右上角通知：
     * 从右滑入 + 底部倒计时条（悬停暂停）+ 可手动关闭 + 自动消失。
     * 说明：
     *   - 绑定成功后会给 <html> 加 yc-toast-on 类，
     *     style3.css 据此隐藏原生红字提示；
     *   - 若本文件被换回原版 WOW（无此模块），该类不会出现，
     *     页面自动回退为原生 #msgpid 提示，报错功能不受影响。
     * ========================================================== */
    ...
    function bootToast() {
        ...
        var DUR = 5000;        /* 停留时长 = 倒计时条走完的时间 */
        var timer = null;      /* 自动关闭定时器 */
        var hideTimer = null;  /* 滑出动画后的清理定时器 */
        var debounce = null;   /* 合并同一次写入触发的多次变异 */
        var suppress = false;  /* 清空源节点时忽略自身触发的变异 */
        ...
        function startBar(toast, ms) {
            ...
            bar.style.width = '100%';
            void bar.offsetWidth;                    /* 强制回流，保证过渡生效 */
